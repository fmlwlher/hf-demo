...
export async function callContract(op, contractAddr, contractJson, methodName, ...params) {
    let contractInst = new op.web3.eth.Contract(contractJson.abi, contractAddr);
    let contractInstMethod = !!params?contractInst.methods[methodName](...params):contractInst.methods[methodName]();
...
export async function sendContract(op, contractAddr, contractJson, methodName, ...params) {
    let contractInst = new op.web3.eth.Contract(contractJson.abi, contractAddr);
    let contractInstMethod = !!params?contractInst.methods[methodName](...params):contractInst.methods[methodName]();
    let gasAmount = await contractInstMethod.estimateGas({from: op.from, value: op.value, gasPrice: 1})
